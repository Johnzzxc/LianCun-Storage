...
func registerAPIRouter(router *mux.Router) {
	...
	for _, bucket := range routers {
		...
		// GetFileDeals
		bucket.Methods(http.MethodGet).Path("/{object:.+}").HandlerFunc(
			collectAPIStats("getfiledeals", maxClients(httpTraceHdrs(api.GetListDealsHandler)))).Queries("deals", "")
		// GetFileRetrieve
		bucket.Methods(http.MethodGet).Path("/{object:.+}").HandlerFunc(
			collectAPIStats("getfileretrieve", maxClients(httpTraceHdrs(api.GetRetrieveHandler)))).Queries("retrieve", "")
		bucket.Methods(http.MethodPut).Path("/{object:.+}").HandlerFunc(
			collectAPIStats("putfileretrieve", maxClients(httpTraceHdrs(api.PutRetrieveHandler)))).Queries("retrieve", "")
