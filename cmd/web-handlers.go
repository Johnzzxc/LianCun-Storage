...
import (
	...
	"errors"
	"fmt"
	"github.com/minio/minio/cloud/model"
	"io"
	"net/http"
	"net/url"
	"os"
...
func (web *webAPIHandlers) MakeBucket(r *http.Request, args *MakeBucketArgs, reply *WebGenericRep) error {
	...
	logger.Info("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
	return nil
}
...
type WebObjectInfo struct {
	...
	//上链状态 1.上传成功  2.已导入filecoin网络 3.上链中 9.上链
	State int16 `json:"state"`
	// 1.热存储	2.冷热存储	3.冷存储
	FileType int16 `gorm:"column:state;type:smallint(4)" json:"fileType"`
	// 标签id
	LableID int64 `json:"lable_id"`
	// 标签名
	Lable string `json:"lableName"`
	// true 可申请，false不可申请
...
func (web *webAPIHandlers) ListObjects(r *http.Request, args *ListObjectsArgs, reply *ListObjectsRep) error {
	...
	for {
		...
		/////////// start /////////////
		encryption := 2
		if len(r.Header["Encryption"]) > 0 {
			if r.Header["Encryption"][0] == "1" {
				encryption = 1
			} else {
				encryption = 0
			}
		...
		page := 0
		pageSize := 0
		if len(r.Header["Page"]) > 0 && len(r.Header["Pagesize"]) > 0 {
			page, err = strconv.Atoi(r.Header["Page"][0])
			pageSize, err = strconv.Atoi(r.Header["Pagesize"][0])
		}

		//files, err := model.CloudFileFactory().GetFileList(args.BucketName + SlashSeparator + args.Prefix,encryption)
		files, count, err := model.CloudFileListFactory().GetFileList(args.BucketName+SlashSeparator+args.Prefix, encryption, page, pageSize)
		if err != nil {
			fmt.Println(err)
		} else {
			fmt.Println("web-handlers:", files)
		}
		reply.Count = count

		MinSize, err := model.CloudDictDataFactory().GetTypeValue("file-min-size")
		if err != nil {
			fmt.Println(err)
		} else {
			fmt.Println("web-handlers:MinSize ", MinSize)
		}
		fileMinSize,err :=  strconv.ParseInt(MinSize, 10, 64)

		//filesMap := make(map[string]model.CloudFile)
		filesMap := make(map[string]model.CloudFileList)
		...
		for _, obj := range filesMap {
			reply.Objects = append(reply.Objects, WebObjectInfo{
				FileId:       obj.ID,
				Key:          obj.FileName,
				LastModified: obj.UpdateTime.Format("2006-01-02 15:04:05"),
				Size:         obj.FileSize,
				ContentType:  obj.ContentType,
				State:        obj.State,
				...
				LableID:      obj.LableID,
				Lable:        obj.Lable,
				IsChain:      (obj.FileSize < fileMinSize) && obj.State == 0,
			})
		}

		fmt.Println("web-handlers,WebObjectInfo0000:", len(reply.Objects))

		PrefixesList, err := model.CloudFileListFactory().GetPrefixes(args.BucketName+SlashSeparator+args.Prefix, encryption)
		...
		fmt.Println("web-handlers,WebObjectInfo00001:", len(reply.Objects))
...
next:
	for _, objectName := range args.Objects {
		...
		if !HasSuffix(objectName, SlashSeparator) && objectName != "" {
			...
			//check 状态是否能够删除
			canDelete := model.CloudFileFactory().CanDeleteFile(args.BucketName, objectName)
			if canDelete == false {
				fmt.Print("==========================不能删除", args.BucketName+"/"+objectName)
				err := errors.New("文件上链中，不能删除")
				return toJSONError(ctx, err)
			}
		...
		for {
			...
			for obj := range objInfoCh {
				...
				//check 状态是否能够删除
				canDelete := model.CloudFileFactory().CanDeleteFile(args.BucketName, obj.Name)
				if canDelete {
					objects = append(objects, objToDel)
				} else {
					fmt.Print("==========================不能删除", args.BucketName+"/"+obj.Name)
				}
			}
...
func (web *webAPIHandlers) Upload(w http.ResponseWriter, r *http.Request) {
	...
	if model.CloudFileFactory().CanPutFile(bucket, object) {
		writeWebErrorResponse(w, errors.New("当前文件已存在！"))
		return
	}
...
// ListDealsRep - list objects response.
type ListDealsRep struct {
	Objects   []WebDealInfo `json:"objects"`
	Writable  bool          `json:"writable"` // Used by client to show "upload file" button.
	UIVersion string        `json:"uiVersion"`
}

type WebDealInfo struct {
	...
	Backup            int64     `gorm:"column:piece_size;type:bigint(64)" json:"backup"`                     // 备份数量
	TransactionPeriod string    `gorm:"column:piece_cid;type:varchar(100)" json:"transaction_period"`        // 交易周期

}

// ListDeals - list deals api.
func (web *webAPIHandlers) ListDeals(r *http.Request, args *ListDealsArgs, reply *ListDealsRep) error {
	ctx := newWebContext(r, args, "WebListDeals")
	reply.UIVersion = browser.UIVersion
	objectAPI := web.ObjectAPI()
	if objectAPI == nil {
		return toJSONError(ctx, errServerNotInitialized)
	...
	for {
		...
		deals, err := model.FileDealFactory().GetDealList(args.FileId)
		if err != nil {
			return &json2.Error{Message: err.Error()}
		}
		count, err := model.TaskDealFactory().CountDeal(args.FileId)
		if err != nil {
			return &json2.Error{Message: err.Error()}
		} else {
			fmt.Print("---> backup count --->", count)
		}

		for _, obj := range deals {

			reply.Objects = append(reply.Objects, WebDealInfo{
				ID:                obj.ID,
				CreateTime:        obj.CreateTime,
				UpdateTime:        obj.UpdateTime,
				FileID:            obj.FileID,
				ProposalCid:       obj.ProposalCid,
				MessageID:         obj.MessageID,
				CloudMinerID:      obj.CloudMinerID,
				Message:           obj.Message,
				Provider:          obj.Provider,
				DealID:            obj.DealID,
				Client:            obj.Client,
				PieceSize:         obj.PieceSize,
				PieceCid:          obj.PieceCid,
				VerifiedDeal:      obj.VerifiedDeal,
				EndDeal:           obj.EndDeal,
				StartDeal:         obj.StartDeal,
				Price:             obj.Price,
				MinerCollateral:   obj.MinerCollateral,
				ClientCollateral:  obj.ClientCollateral,
				State:             obj.State,
				StateName:         obj.StateName,
				RootID:            obj.RootID,
				DelFlag:           obj.DelFlag,
				Backup:            count,
				TransactionPeriod: obj.SpaceDate(),
			})
		}
...
// SetRetrieveArgs - set bucket policy args.
type SetRetrieveArgs struct {
	FileId     int64  `json:"fileId"`
	BucketName string `json:"bucketName"`
	Prefix     string `json:"prefix"`
}

// SetRetrieve - 根据文件id增加检索信息
func (web *webAPIHandlers) SetRetrieve(r *http.Request, args *SetRetrieveArgs, reply *WebGenericRep) error {
	ctx := newWebContext(r, args, "WebSetRetrieve")
	objectAPI := web.ObjectAPI()
	reply.UIVersion = browser.UIVersion
	...
	minerId, dealId, err := model.FileDealFactory().GetMinerIdDealId(args.FileId)
	if err != nil {
		return toJSONError(ctx, err, strconv.FormatInt(args.FileId, 10))
	} else {
		err := model.TaskRetrieveFactory().AddRetrieve(args.FileId, minerId, dealId)
		if err != nil {
			return toJSONError(ctx, err, strconv.FormatInt(args.FileId, 10))
		}
		return nil
...
// ListDealsRep - list objects response.
type GetRetrieveRep struct {
	Object    model.CloudTaskRetrieve `json:"object"`
	UIVersion string                  `json:"uiVersion"`
}

// SetRetrieve - 根据文件id查询检索信息
func (web *webAPIHandlers) GetRetrieve(r *http.Request, args *SetRetrieveArgs, reply *GetRetrieveRep) error {
	ctx := newWebContext(r, args, "WebGetRetrieve")
	objectAPI := web.ObjectAPI()
	reply.UIVersion = browser.UIVersion
	...
	result, err := model.TaskRetrieveFactory().GetRetrieve(args.FileId)
	if err != nil {
		return toJSONError(ctx, err, strconv.FormatInt(args.FileId, 10))
	}
	reply.Object = result
