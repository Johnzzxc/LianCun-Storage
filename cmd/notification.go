...
func sendEvent(args eventArgs) {
	...
	switch args.EventName.String() {
	case "s3:ObjectCreated:Put", "s3:ObjectCreated:CompleteMultipartUpload": //新增文件
		accessKey := args.ReqParams["accessKey"]
		platformId, err := model.CloudPlatformInfoFactory().GetPlatformBySecretKey(accessKey)
		if err != nil {
			fmt.Println("查询platformId失败", err)
		} else {
			subjectId, err := model.CloudSubjectDataFactory().GetSubjectID(info.Bucket)
			if err != nil {
				fmt.Println("查询数据主体id失败", err)
			} else {
				encryption := false
				if args.ReqParams["Encryption"] == "1" {
					encryption = true
				}
				lable, e := strconv.ParseInt(args.ReqParams["Lableid"], 10, 64)
				if e != nil {
					fmt.Println("lableId:", e)
					lable = 0
				}

				err := model.CloudFileFactory().PutFile(platformId, subjectId, info.Bucket, info.Name, info.Size, info.ETag, info.ContentType, encryption, lable)
				if err != nil {
					fmt.Println("上传文件插入表失败", err)
				}
			}

		}
	case "s3:ObjectRemoved:Delete":
		err := model.CloudFileFactory().DeleteFile(info.Bucket, info.Name)
		if err != nil {
			fmt.Println("删除文件插入表失败", err)
		}
	case "s3:BucketCreated:*":
		...
		if err != nil {
			...
			err := model.CloudSubjectDataFactory().CreateBucket(args.BucketName,platformId)
			if err != nil {
				fmt.Println("创建bucket插入表失败", err)
			}
		}
	}
	fmt.Println("♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥ sendEvent ♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥")
