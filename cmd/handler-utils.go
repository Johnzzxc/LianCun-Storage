...
func extractReqParams(r *http.Request) map[string]string {
	...
	if len(r.Header["Encryption"]) > 0 {
		Encryption = r.Header["Encryption"][0]
	}

	Lableid := "0"

	if len(r.Header["Lableid"]) > 0 {
		Lableid = r.Header["Lableid"][0]
	}

	page := "0"
	pageSize := "0"
	if len(r.Header["page"]) > 0 {
		page = r.Header["page"][0]
		fmt.Println("page：",page)
	}
	if len(r.Header["pagesize"]) > 0 {
		pageSize = r.Header["pagesize"][0]
