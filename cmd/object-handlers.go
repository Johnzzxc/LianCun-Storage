...
type FileDealReply struct {
	model.CloudFileDeal
	Backup int64 //备份数量
}

// GetListDealsHandler - GET file deals
func (api objectAPIHandlers) GetListDealsHandler(w http.ResponseWriter, r *http.Request) {
	ctx := newContext(r, w, "GetListDeals")
	defer logger.AuditLog(ctx, w, r, mustGetClaimsFromToken(r))
	...
	// Get file deals
	deals, err := model.FileDealFactory().GetDealList(opts.FileId)
	if err != nil {
		writeErrorResponse(ctx, w, toAPIError(ctx, err), r.URL, guessIsBrowserReq(r))
		return
	}
	count, err := model.TaskDealFactory().CountDeal(opts.FileId)
	...
	var result []FileDealReply = make([]FileDealReply, 0)
	for _, obj := range deals {

		result = append(result, FileDealReply{
			obj,
			count,
		})
	}
	...
	json.Indent(&out, bs, "", "\t")
	fmt.Printf("GetFileDeals=%v\n", out.String())

	writeSuccessResponseXML(w, encodeResponse(result))
}

// GetRetrieveHandler - GET file retrieve
func (api objectAPIHandlers) GetRetrieveHandler(w http.ResponseWriter, r *http.Request) {
	ctx := newContext(r, w, "GetRetrieve")
	defer logger.AuditLog(ctx, w, r, mustGetClaimsFromToken(r))
	...
	// Get file retrieve
	result, err := model.TaskRetrieveFactory().GetRetrieve(opts.FileId)
	if err != nil {
		writeErrorResponse(ctx, w, toAPIError(ctx, errFileRetrieveNotFound_), r.URL, guessIsBrowserReq(r))
		return
	}
	...
	var out bytes.Buffer
	json.Indent(&out, bs, "", "\t")
	fmt.Printf("GetRetrieve=%v\n", out.String())

	writeSuccessResponseXML(w, encodeResponse(result))
}

// PutRetrieveHandler - put file retrieve
func (api objectAPIHandlers) PutRetrieveHandler(w http.ResponseWriter, r *http.Request) {
	ctx := newContext(r, w, "PutRetrieve")
	defer logger.AuditLog(ctx, w, r, mustGetClaimsFromToken(r))
	...
	minerId,dealId, err := model.FileDealFactory().GetMinerIdDealId(opts.FileId)
	if err != nil {
		writeErrorResponse(ctx, w, toAPIError(ctx, errFileRetrieveNotFound_), r.URL, guessIsBrowserReq(r))
		return
	} else {
		err := model.TaskRetrieveFactory().AddRetrieve(opts.FileId, minerId, dealId)
		if err != nil {
			writeErrorResponse(ctx, w, toAPIError(ctx, errFileRetrieveWorking_), r.URL, guessIsBrowserReq(r))
			return
		}
	}
	if opts.VersionID != "" {
