...
func generateListObjectsV2Response(bucket, prefix, token, nextToken, startAfter, delimiter, encodingType string, fetchOwner, isTruncated bool, maxKeys int, objects []ObjectInfo, prefixes []string, metadata bool) ListObjectsV2Response {
	...
	/////////// start /////////////
	files, err := model.CloudFileFactory().GetFileList(bucket+SlashSeparator+prefix, 2)
	if err != nil {
		fmt.Println(err)
	} else {
		fmt.Println("api-response:", files)
		fmt.Println(files)
	}
	...
	for _, object := range filesMap {
		...
		fmt.Println("--------------------------------------------")
		fmt.Println(object)
		content.Key = s3EncodeName(object.FileName, encodingType)
		content.LastModified = object.UpdateTime.UTC().Format(iso8601TimeFormat)
		if object.FileHash != "" {
			content.ETag = "\"" + object.FileHash + "\""
		}
		content.Size = object.FileSize
		//todo ---- 存储类型
		//if object.StorageClass != "" {
		//	content.StorageClass = object.StorageClass
		//} else {
		...
		//}
		content.Owner = owner
		content.FileId = object.ID
		content.ContentType = object.ContentType
		content.State = object.State
		content.FileType = object.GetFileType()
		fmt.Println(content)
		fmt.Println("--------------------------------------------")
		//todo ----- metadata
		//if metadata {
...
func writeResponse(w http.ResponseWriter, statusCode int, response []byte, mType mimeType) {
	...
	fmt.Println("response", *(*string)(unsafe.Pointer(&response)))
	fmt.Println("🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅🍅")
